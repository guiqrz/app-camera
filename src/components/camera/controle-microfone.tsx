...
export function ControleMicrofone({
  ...
  return (
    <div className="border-border-default bg-surface shadow-card flex flex-col gap-3 rounded-2xl border p-5">
      ...
      {/* Faixa de gravacao: cor de perigo cheia, igual ao alerta de dispersao.
          So' aparece com confirmacao POSITIVA da captura. */}
      {gravando && (
        <div
      ...
      <button
        ...
        style={{
          borderColor: gravando ? "var(--danger)" : "var(--border)",
          background: gravando ? "var(--danger-bg)" : "var(--surface)",
        }}
      >
        <span className="flex items-center gap-2.5">
          <span
            aria-hidden
            style={{ color: gravando ? "var(--danger-fg)" : "var(--text-muted)" }}
          >
            {gravando ? <IconMicrofone size={20} /> : <IconMicrofoneCortado size={20} />}
          </span>
          <span
            className="text-sm font-extrabold"
            style={{ color: gravando ? "var(--danger-fg)" : "var(--text)" }}
          >
            {aguardando
              ? "Aplicando…"
              : gravando
        ...
        <span
          ...
          style={{ background: gravando ? "var(--danger)" : "var(--surface-2)" }}
        >
          <span
