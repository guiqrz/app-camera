"use client";

import { useRouter } from "next/navigation";
import { useTransition } from "react";

import { IconSeta, IconTurma } from "@/components/ui/icons";
import type { Turma } from "@/lib/types";

type SeletorTurmaRelatorioProps = {
  turmas: Turma[];
  turmaAtualId: number;
};

/** Troca a turma do relatorio geral, navegando para /relatorios/turma/{id}. */
export function SeletorTurmaRelatorio({
  turmas,
  turmaAtualId,
}: SeletorTurmaRelatorioProps) {
  const router = useRouter();
  const [pendente, iniciarTransicao] = useTransition();

  if (turmas.length === 0) return null;

  return (
    <label
      className="border-border-default bg-surface flex items-center gap-2.5 rounded-xl border px-4 py-2.5"
      style={{ opacity: pendente ? 0.6 : 1 }}
    >
      <span className="flex-none" style={{ color: "var(--primary)" }}>
        <IconTurma />
      </span>
      <span className="sr-only">Escolher turma do relatório</span>
      <select
        value={turmaAtualId}
        disabled={pendente}
        onChange={(evento) => {
          const id = evento.target.value;
          iniciarTransicao(() => router.push(`/relatorios/turma/${id}`));
        }}
        className="text-text cursor-pointer appearance-none bg-transparent pr-1 text-sm font-bold outline-none"
      >
        {turmas.map((turma) => (
          <option key={turma.id} value={turma.id}>
            {turma.nome}
          </option>
        ))}
      </select>
      <span className="text-text-muted flex-none" aria-hidden>
        <IconSeta />
      </span>
    </label>
  );
}
